...
public interface MemoService {
    ...
    //3. 삭제대기 상태인 메모만 보겠다.
    List<Memo> getMemoList(User user, Search search) throws Exception;
    ...
    //메모를 도메인에 부착
    void addMemoAttach(int memoNo,MemoAttachable memoAttachable) throws Exception;

    //메모를 도메인에서 분리
    void deleteMemoAttach(int memoNo,MemoAttachable memoAttachable) throws Exception;

    //메모에 부착된 도메인을 변경
    void updateMemoAttach(int memoNo,MemoAttachable memoAttachable) throws Exception;

    //메모에 부착된 도메인을 조회
    MemoAttachable getMemoAttach(int memoNo) throws Exception;
